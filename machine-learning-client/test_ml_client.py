...
def mock_face_detector():
    ...
    with patch('cv2.CascadeClassifier', autospec=True) as MockClass:
...
def mock_emotion_detector():
    ...
    with patch('fer.FER', autospec=True) as MockClass:
        instance = MockClass.return_value
...
@patch('cv2.cvtColor')
def test_full_pipeline_integration(mock_cvt_color, encoded_image, mock_face_detector, mock_emotion_detector):
